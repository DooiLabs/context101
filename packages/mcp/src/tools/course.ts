...
const _courseLessonSchema = z.object({
  lessonName: z
    .string()
    .describe("Name of the specific lesson to start. It must match the exact lesson name."),
});
...
async function registerUser(
  ...
  return (await response.json()) as { success: boolean; id: string; key: string; message: string };
}
        ...
        res.on("end", () => {
          ...
          try {
            ...
            if (parsedData.success) {
              ...
              reject(new Error(parsedData.message || "Failed to update course state"));
            }
          } catch (error) {
            reject(new Error(`Failed to parse response: ${error}`));
          }
...
async function updateCourseStateOnServer(deviceId: string, state: CourseState): Promise<void> {
  const response = await fetch("https://mastra.ai/api/course/update", {
    ...
    headers: {
      "Content-Type": "application/json",
      "x-mastra-course-key": (await getDeviceCredentials())?.key || "",
    },
    body: JSON.stringify({ id: deviceId, state }),
  });

  if (!response.ok) {
    throw new Error(`Failed to update course state: ${response.statusText}`);
  }
}

// Create a function to save the course state
async function saveCourseState(state: CourseState, deviceId: string | null = null): Promise<void> {
  ...
  if (deviceId) {
    try {
      ...
      // Fall back to local server if remote update fails
      await updateCourseStateOnServerLocally(deviceId, state);
    }
  }
}

// Create a function to load the course state
...
// Get the course state path
async function getCourseStatePath(): Promise<string> {
  const cacheDir = path.join(os.homedir(), ".cache", "mastra", "course");
  await fs.mkdir(cacheDir, { recursive: true });
  return path.join(cacheDir, "state.json");
}

// Scan the course content directory for lessons and steps
async function scanCourseContent(): Promise<CourseState> {
  ...
  const lessons = await Promise.all(
    lessonDirs
      ...
      .map(async (lessonDir) => {
        ...
        const steps = await fs.readdir(lessonPath);
        const stepList = steps
          .filter((file) => file.endsWith(".md"))
          .map((stepFile) => ({
            name: stepFile.replace(/^\d+-/, "").replace(".md", ""),
            status: 0,
          }));

        return {
          name: lessonDir.replace(/^\d+-/, ""),
          status: 0,
          steps: stepList,
        };
      })
  ...
  const filteredLessons = lessons.filter(Boolean) as CourseState["lessons"];

  return {
    currentLesson: filteredLessons[0]?.name || "",
    lessons: filteredLessons,
  };
}

// Merge previous course state with new content
async function mergeCourseStates(currentState: CourseState, latestState: CourseState): Promise<CourseState> {
  const mergedLessons = latestState.lessons.map((newLesson) => {
    const existingLesson = currentState.lessons.find((lesson) => lesson.name === newLesson.name);

    if (!existingLesson) {
      return newLesson;
    }

    const lessonStatus = existingLesson.status;

    const mergedSteps = newLesson.steps.map((newStep) => {
      const existingStep = existingLesson.steps.find((step) => step.name === newStep.name);
      if (!existingStep) {
        return newStep;
      }
      return {
    ...
    try {
      ...
      if (courseState) {
        ...
        const newLessons = latestCourseState.lessons.filter(
          (newLesson) =>
            !previousState.lessons.some((oldLesson: { name: string }) => oldLesson.name === newLesson.name)
        );
...
export const getMastraCourseStatus = {
  ...
  execute: async (_args: Record<string, never>) => {
    try {
      ...
      statusReport += `- Lessons: ${completedLessons}/${totalLessons} completed (${Math.round(
        (completedLessons / totalLessons) * 100
      )}%)\n`;
      statusReport += `- Steps: ${completedSteps}/${totalSteps} completed (${Math.round(
  ...
  execute: async (args: z.infer<typeof _courseLessonSchema>) => {
    try {
      ...
      const firstIncompleteStep =
        targetLesson.steps.find((step) => step.status !== 2) || targetLesson.steps[0];
